...
# Connect to Redis db -> assuming the Redis container is named 'redis'
# if not using Redis as a container then modify the code accordingly
...
# Store the Q&A list in Redis
for question, answer in qa_list.items():
...
async def ask_question(question: Question):
    ...
    answer = redis_client.get(normalized_question)
    if answer:
        return {"answer": answer.decode('utf-8')}
    else:
